...
function index(key: string, length: number, append = false): number {
  ...
  if (value > length || (!append && value === length)) throw new Error('Array index out of bounds')
  return value
}

function child(value: unknown, key: string): unknown {
  if (Array.isArray(value)) return value[index(key, value.length)]
  if (value === null || typeof value !== 'object' || !Object.hasOwn(value, key)) throw new Error('JSON pointer does not resolve')
...
function equal(left: unknown, right: unknown): boolean {
  ...
  return keys.length === Object.keys(right).length && keys.every((key) => Object.hasOwn(right, key) && equal(Reflect.get(left, key), Reflect.get(right, key)))
}

/** RFC 6902, applied atomically. A rejected operation leaves the source untouched. */
export function applyAGUIPatch(source: unknown, patch: Array<JsonPatchOperation>): unknown {
  let document = copy(source)
  const get = (path: Array<string>): unknown => path.reduce(child, document)
  const write = (path: Array<string>, value: unknown, operation: 'add' | 'replace' | 'remove') => {
    if (!path.length) {
    ...
    if (Array.isArray(parent)) {
      ...
      if (parent === null || typeof parent !== 'object') throw new Error('JSON pointer parent is not a container')
      if (operation !== 'add' && !Object.hasOwn(parent, key)) throw new Error('JSON pointer does not resolve')
      if (operation === 'remove') Reflect.deleteProperty(parent, key)
      // Define rather than assign: __proto__ is JSON data, never a setter.
      else Object.defineProperty(parent, key, { value: copy(value), enumerable: true, writable: true, configurable: true })
    }
  }
  for (const operation of patch) {
    const path = segments(operation.path)
    switch (operation.op) {
      case 'add': case 'replace': write(path, operation.value, operation.op); break
      case 'remove': write(path, undefined, 'remove'); break
      case 'test': if (!equal(get(path), operation.value)) throw new Error('JSON Patch test failed'); break
      case 'copy': write(path, get(segments(operation.from)), 'add'); break
      case 'move': {
        const from = segments(operation.from)
        if (from.length < path.length && from.every((key, position) => path[position] === key)) throw new Error('Cannot move a value into its descendant')
