...
import {
  validateAGUIEvent,
  validateAGUIInput,
} from '../src/utilities/ag-ui-validation'
...
const fixtures: Array<Fixture> = readdirSync(directory)
  .filter((file) => file.endsWith('.json'))
  .map((file) => JSON.parse(readFileSync(new URL(file, directory), 'utf8')))
const boundaryFixtures = Object.values(fixtures).filter(
  (fixture) =>
    !fixture.name.startsWith('era-') &&
    fixture.name !== 'unknown-enum-value-role-fatal',
)
...
describe('AG-UI 1.0 protocol boundary', () => {
  it.each(boundaryFixtures)('$name', (fixture) => {
    ...
    const processor = new StreamProcessor({
      initialMessages: fixture.input?.messages?.map(
    ...
    const consume = () => {
      for (const event of fixture.stream) {
        for (const next of stream.push(event)) {
          events.push(next)
          processor.processChunk(next)
        }
      }
    }
    ...
    if (fixture.expect.messages !== undefined)
      expect(messages).toMatchObject(fixture.expect.messages)
    if (fixture.expect.messageCount !== undefined)
      expect(messages).toHaveLength(fixture.expect.messageCount)
    if (Object.hasOwn(fixture.expect, 'state'))
      expect(processor.getAgentState()).toEqual(fixture.expect.state)
    if (fixture.expect.eventTypes)
      expect(events.map((event) => event.type)).toEqual(
        fixture.expect.eventTypes,
      )
    for (const type of fixture.expect.eventTypesAbsent ?? [])
      expect(events.map((event) => event.type)).not.toContain(type)
    for (const [path, value] of Object.entries(fixture.expect.eventPaths ?? {}))
      expect(at(events, path), path).toEqual(value)
    for (const path of fixture.expect.eventAbsentPaths ?? [])
      expect(at(events, path), path).toBeUndefined()
  ...
  it('strips unknown enum values rather than rejecting a future role', () => {
    expect(
      validateAGUIEvent(
        { type: 'TEXT_MESSAGE_START', messageId: 'm', role: 'future' },
        () => {},
      ),
    ).toEqual({ type: 'TEXT_MESSAGE_START', messageId: 'm' })
  })
  ...
  it('validates nested request fields without coercion', () => {
    ...
    ).toThrow('/messages/0/toolCalls/0/function/arguments')
  })
})
