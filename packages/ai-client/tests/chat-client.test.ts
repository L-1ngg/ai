...
describe('ChatClient', () => {
  ...
  describe('subscribe/send connection mode', () => {
            ...
            queuedChunks.push({
              ...
              chunks: [
                ...
                ...createTextChunks(
                  ...
                  .filter((chunk) => chunk.type !== 'RUN_STARTED'),
              ],
            })
            wakeSubscriber?.()
            wakeSubscriber = null
          ...
          async (
            ...
            queuedChunks.push({
              ...
              chunks: [
                ...
                { type: EventType.TEXT_MESSAGE_END, messageId: messageId },
                {
                  type: EventType.RUN_FINISHED,
                  threadId: runContext?.threadId ?? 'thread-1',
                  runId:
    ...
    it('should not add internal threadId or runId to public connect chunks', async () => {
      ...
      const client = new ChatClient({
        connection: {
          async *connect() {
            yield* createTextChunks('Hello', 'public-message')
          },
        },
        onChunk,
      })
  ...
  describe('append', () => {
    ...
    it('keeps append pending when intermediate tool_calls is in TanStack metadata', async () => {
      const adapter: ConnectConnectionAdapter = {
        async *connect(_messages, _data, _signal, ctx) {
          const runId = ctx?.runId ?? 'run-1'
          const threadId = ctx?.threadId ?? 'thread-1'
  ...
  describe('custom events', () => {
    ...
    it('should handle custom events with no data', async () => {
      ...
      expect(onCustomEvent).toHaveBeenCalledWith('simple-notification', null, {
        toolCallId: undefined,
      })
    })
  ...
  describe('concurrent runs and reconnect correctness', () => {
    it('concurrent runs should not produce duplicate messages or corrupt content', async () => {
      ...
      push({ type: EventType.TEXT_MESSAGE_END, messageId: 'msg-b' })
      push({
        type: EventType.RUN_FINISHED,
        runId: 'run-b',
        threadId: 'thread-1',
      ...
      push({ type: EventType.TEXT_MESSAGE_END, messageId: 'msg-a' })
      push({
        type: EventType.RUN_FINISHED,
        runId: 'run-a',
        threadId: 'thread-1',
    ...
    it('reconnect with initialMessages should not duplicate assistant message on content arrival', async () => {
      ...
      push(
        ...
        { type: EventType.TEXT_MESSAGE_END, messageId: 'asst-1' },
        {
          type: EventType.RUN_FINISHED,
          runId: 'run-1',
          threadId: 'thread-1',
